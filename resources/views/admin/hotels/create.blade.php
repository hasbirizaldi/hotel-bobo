...
@endif --}}
                <form method="POST" action="{{ route('admin.hotels.store') }}" enctype="multipart/form-data"> 
                    ...
                    <div class="mt-4">
                        <x-input-label for="link_gmap" :value="__('Link Google Map')" />
                        <x-text-input id="link_gmap" class="block mt-1 w-full" type="text" name="link_gmap" :value="old('link_gmap')" required autofocus autocomplete="link_gmap" />
                        <x-input-error :messages="$errors->get('link_gmap')" class="mt-2" />
                    </div>
