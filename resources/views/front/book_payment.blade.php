...
@section('content')
  ...
  <section id="content" class="max-w-[640px] w-full min-h-screen mx-auto flex flex-col bg-[#F8F8F8] overflow-x-hidden pb-[120px] relative">
    ...
    <div class="relative z-10 flex flex-col gap-6 mt-[60px]">
      ...
      <form action="{{ route('front.hotel.book.payment.store', $hotel_booking) }}" method="POST" id="Details" class="group result-card-container flex flex-col gap-6" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div id="Transfer-details" class="bg-white rounded-xl flex flex-col mx-[18px] p-4 gap-4">
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="flex flex-col gap-[2px]">
                <p class="font-semibold text-sm leading-[21px] text-[#4041DA]">
                  {{ $hotel_booking->checkin_at->format('d M Y') }} - {{ $hotel_booking->checkout_at->format('d M Y') }} ({{ $hotel_booking->total_day }} Night)
                </p>
                <p class="font-semibold">{{ $hotel_booking->room->name }}</p>
                <p class="font-medium text-sm leading-[21px] text-[#757C98]">Max.2 Adult /Room</p>
              </div>
            </div>
          </div>
          <div class="input-container flex flex-col gap-2">
            <div class="flex flex-col gap-[2px]">
              ...
              <p class="font-semibold">PT. Abimanyu Sejahtera</p>
            </div>
            <div class="flex items-center gap-[10px] p-[12px_16px] rounded-lg bg-[#F8F8F8]">
              ...
              <p class="copy font-semibold flex flex-1">444 434 234 344</p>
